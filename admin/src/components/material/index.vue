<template>
  <div class="material" v-loading="pager.loading">
    ...
    <div class="material__center flex flex-col">
      ...
      <div class="material-center__content flex flex-col flex-1 mb-1 min-h-0">
          ...
          <ul class="file-list flex flex-wrap mt-4">
            ...
            >
              <del-wrap @close="batchFileDelete([item.id])">
                <file-item
                  :uri="VITE_FILE_PREFIX + item.path"
                  :file-size="fileSize"
                  :type="type"
        ...
        >
          ...
          <el-table-column label="名称" min-width="100" show-overflow-tooltip>
            <template #default="{ row }">
              <el-link @click.stop="handlePreview(row.path)" :underline="false">
                {{ row.name }}
              </el-link>
            </template>
          </el-table-column>
            ...
            <template #default="{ row }">
              ...
              <div class="inline-block">
                <el-button
                  type="primary"
                  link
                  @click.stop="handlePreview(row.path)"
                >
                  查看
                </el-button>
              </div>
      ...
      <div
        class="material-center__footer flex justify-between items-center mt-2"
      >
        <div class="flex">
...
const VITE_FILE_PREFIX = import.meta.env.VITE_FILE_PREFIX;
...
const handlePreview = (url: string) => {
  previewUrl.value = VITE_FILE_PREFIX + url;
