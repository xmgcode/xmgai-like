...
public class AiSwapTaskJob {
    ...
    public void handle() {
        ...
        List<AiTask> taskList = aiTaskMapper.selectList(new QueryWrapper<AiTask>()
                .eq("status", 0)
                .eq("type", AiEnum.MjFaceswap.getValue())
                .orderByAsc("create_time"));
        log.info("当前换脸任务数：" + taskList.size());
        for(int i=0;i<taskList.size();i++){
            ...
            if(null != object && "200".equals(object.get("code").toString())){
                ...
                if("100%".equals(resObj.get("progress").toString())){
                    ...
                    aiArtcode.setAiName(AiEnum.MjFaceswap.getName());
                    aiArtcode.setAiType(AiEnum.MjFaceswap.getValue());
