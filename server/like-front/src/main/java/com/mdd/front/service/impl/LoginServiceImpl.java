...
public class LoginServiceImpl implements ILoginService {
    ...
    public void register(String username, String password, Integer terminal) {
        ...
        user.setApiKey("AK-" + ToolUtils.getApiKey());
        user.setSalt(salt);
    ...
    @Override
    public void registerMobile(String username, String password, String mobile, Integer terminal) {
        User model = userMapper.selectOne(new QueryWrapper<User>()
                .select("id,sn,username")
                .eq("mobile", mobile)
                .eq("is_delete", 0)
    ...
    public LoginTokenVo mobileLogin(String mobile, String code, Integer terminal) {
        ...
        //如果手机号没注册就重新注册
        if(null == user){
            String userName = "用户" + SnowIdUtil.getStrUUid();
            String password = SnowIdUtil.getStrUUid();
            //注册
            registerMobile(userName,password,mobile,terminal);

            //再次根据手机号查询用户
            // 查询手机号
            User userMobile = userMapper.selectOne(new QueryWrapper<User>()
                    .select("id,username,mobile,is_disable,is_new")
                    .eq("mobile", mobile)
                    .eq("is_delete", 0)
                    .last("limit 1"));
    ...
    @Override
    public User validatePhone(String mobile) {
        // 查询手机号
        User user = userMapper.selectOne(new QueryWrapper<User>()
                .select("id,username,mobile,is_disable,is_new")
                .eq("mobile", mobile)
    ...
    @Override
    public LoginTokenVo getWechatOauthInfo(WechatOauthInfo wechatOauthInfo) {
        Integer terminal = LikeFrontThreadLocal.getTerminal();
        User model = userMapper.selectOne(new QueryWrapper<User>()
                .select("id,sn,username")
                .eq("username", wechatOauthInfo.getOpenid())
                .eq("is_delete", 0)
                .last("limit 1"));

        if(null != model){
            //"已注册，不做处理,直接登录"
            return this.__loginToken(model.getId(), model.getMobile(), model.getIsNew(), terminal);
        }
        ...
        User user = new User();
        ;
        Long longUUid = SnowIdUtil.getLongUUid();
        int id = Integer.parseInt(String.valueOf(longUUid));
        user.setId(id);
        user.setSn(sn);
        user.setNickname(wechatOauthInfo.getNickname());
        user.setUsername(wechatOauthInfo.getOpenid());
        user.setPassword(pwd);
        user.setApiKey("AK-" + ToolUtils.getApiKey());
        user.setSalt(salt);
        user.setAvatar(wechatOauthInfo.getHeadimgurl());
        //7表示微信注册
        user.setChannel(7);
        user.setIsNew(1);
        user.setCreateTime(System.currentTimeMillis() / 1000);
        ...
        //注册后完成登录
        return this.__loginToken(id, user.getMobile(), user.getIsNew(), terminal);
    }
