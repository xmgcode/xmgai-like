<template>
  <div class="container">
    ...
    <div class="left-titles">
      <!-- 第一个标题 -->
      <el-row>
        <el-col :span="24" class="title">一、上传二维码</el-col>
      </el-row>
      ...
      <!-- 第二个标题 -->
      <el-row>
        <el-col :span="24" class="title">二、选择模板</el-col>
      </el-row>
      ...
      <el-row>
        <el-col :span="24">
          <el-button class="title-wrapper" @click="handleSelectTemplate" type="primary">选择模板</el-button>
        </el-col>
      </el-row>
    </div>
    ...
    <div class="right-title">
      <!-- 第三个标题 -->
      <el-row>
        <el-col :span="24" class="threetitle">独一无二的作品</el-col>
      </el-row>
      <el-row>
        <el-col :span="24">
    ...
    <!-- ElDialog -->
    <ElDialog v-model="dialogVisible" title="二维码模板" width="55%" append-to-body :close-on-click-modal="true">
...
import { ref } from "vue";
import { ElUpload, ElImage, ElDialog, ElRow, ElCol, ElButton } from "element-plus";
...
const handleFileChange = (file, fileList) => {
  if (!file.raw) {
    console.error('没有选择文件或文件对象不包含raw属性');
    return;
  }
...
const handleSubmit = async () => {
  paramsCreate.qr_image = base64Image.value;
  paramsCreate.template_id = modelId.value;
  if(!base64Image.value){
    feedback.msgError("请先上传二维码！");
    return;
  }
  if(!modelId.value){
    feedback.msgError("请选择模板！");
    return;
  }
  ...
  loading.value = true;
  const ctdata = await createArtcode(paramsCreate);
  console.log("ctdata",ctdata);
  if(ctdata.code === 0){
    feedback.msgError(ctdata.data.msg);
    butMsg.value = "立即生成（消耗6点数）";
    loading.value = false;
    return;
  }
  ...
  if(ctdata.code === 1){
    ...
    //获得图片ID
    paramsCode.imageId = ctdata.data;
...
const getQrCodeUrl = async () => {
    const gcdata = await getQrCode(paramsCode);
    //二维码生成中
    if(gcdata.code === 0){
      butMsg.value = "拼命生成中，请稍后！";
    }
    //二维码生成完成
    if(gcdata.code === 1){
      console.info("gcdata12312",gcdata.data);
      resultpic.value =  gcdata.data;
