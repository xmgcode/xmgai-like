    ...
    <div class="border-b border-br pb-5">
      <div class="user-info">
        <div class="section">
          <div>
            <el-row>
              <el-col :span="24" class="points-title">
                当前可用点数:
                <span>{{myRemainPoints}}</span>
              </el-col>
            </el-row>
          </div>
          <!-- 使用记录的内容 -->
        ...
        <div class="section">
          <div class="recorse-list">
            <el-row>
              <el-col :span="24" class="recorse-title">充值记录</el-col>
            </el-row>
            <el-table :data="myUserRecords" border  height="250" style="width: 100%">
              <el-table-column prop="money" label="充值金额" width="180" />
              <el-table-column prop="allpoints" label="本次获取点数" width="180" >
                 <template #default="{ row }">
                  {{ row.obtainPoints + row.givePoints }}
                </template>
              </el-table-column>
              
              <el-table-column prop="obtainPoints" label="兑换点数" width="180" />
              <el-table-column prop="givePoints" label="赠送点数" width="180" />
...
const recordList = async () => {
  try {
    const data = await xmgai.getChargeRecords(); // 修正函数调用，添加括号
